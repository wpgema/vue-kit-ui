        ...
        <label :for="name" class="flex items-center gap-3 cursor-pointer"
            ...
            <div class="relative flex items-center justify-center w-5 h-5 rounded-md border transition-all duration-150"
                ...
                <input type="checkbox" :id="name" :name="name"
                    class="appearance-none absolute w-full h-full cursor-pointer rounded-md" :checked="internalChecked"
                    @change="toggleCheck" :disabled="disabled" :readonly="readonly" />
                <svg v-if="internalChecked" xmlns="http://www.w3.org/2000/svg"
                    class="w-3 h-3 text-white bg-gray-800 rounded-sm" viewBox="0 0 20 20" fill="currentColor">
